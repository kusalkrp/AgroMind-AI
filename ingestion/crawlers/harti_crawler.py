"""
HARTI Crawler — scrapes harti.gov.lk for agricultural research publications.
...
HARTI_PUBLICATION_PATHS = [
    "/index.php/en/research-reports",
    "/index.php/en/publications",
    "/index.php/en/market-information",
]
...
async def _fetch_page_links(page: Page, url: str) -> list[str]:
    ...
    for anchor in anchors:
        ...
        if _is_pdf_link(href):
            full_url = urljoin(HARTI_BASE_URL, href) if href.startswith("/") else href
            if HARTI_BASE_URL in full_url:
...
async def crawl_harti(output_dir: Path | None = None) -> list[Path]:
    """
    Crawl harti.gov.lk and download all research PDFs.

    Args:
        output_dir: Destination folder. Defaults to settings.raw_data_dir / "harti".

    Returns:
        List of paths to downloaded PDF files.
    """
    output_dir = output_dir or (settings.raw_data_dir / "harti")
    ...
    async with async_playwright() as pw:
        ...
        for path in HARTI_PUBLICATION_PATHS:
            url = urljoin(HARTI_BASE_URL, path)
            try:
    ...
    logger.info(f"HARTI crawl complete. {len(downloaded)} PDFs saved to {output_dir}")
    return downloaded


if __name__ == "__main__":
    asyncio.run(crawl_harti())
