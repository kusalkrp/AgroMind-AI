...
Mocks: call_gemini (via config.gemini), temporal_retriever.
...
class TestRiskNode:
    ...
    def test_risk_node_sets_risk_assessment(self):
        ...
        with patch("agents.risk_agent.get_weather_risk_context", return_value=MOCK_WEATHER), \
             patch("agents.risk_agent.call_gemini", return_value=json.dumps(MOCK_RISK_RESPONSE)):
    ...
    def test_risk_node_handles_llm_failure_gracefully(self):
        """If call_gemini raises an exception, risk_node should return a safe default state."""
        state = _make_state(semantic_context=[])

        with patch("agents.risk_agent.get_weather_risk_context", return_value=MOCK_WEATHER), \
             patch("agents.risk_agent.call_gemini", side_effect=Exception("API quota exceeded")):
            from agents.risk_agent import risk_node
            result = risk_node(state)
